...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ evento?: string }> }) {
  ...
  let eventQuery = admin.from("events").select("id, title, event_date, invite_token, grams_per_person, beer_liters_per_drinker, status").eq("owner_id", user.id);
  ...
  return <main><AdminHeader active="resumo" eventId={event?.id} />
    ...
    {event && <InviteManager eventId={event.id} invitations={invitationData ?? []} readOnly={event.status==="closed"} />}
