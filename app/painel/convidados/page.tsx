// Reúne edição de convidados e criação de convites em uma área protegida.
...
export default async function GuestsPage({ searchParams }: { searchParams: Promise<{ evento?: string }> }) {
  ...
  let eventQuery = context.database.from("events").select("id,title,status,invite_token").eq("owner_id", context.user.id);
