...
export default async function GuestsPage({ searchParams }: { searchParams: Promise<{ evento?: string }> }) {
  ...
  let eventQuery = context.database.from("events").select("id,title,status").eq("owner_id", context.user.id);
  ...
  return <main><AdminHeader active="convidados" eventId={event?.id}/><section className="page-heading"><span className="eyebrow">{event?.title??"EVENTO"}</span><h1>Convidados</h1><p>Crie convites e corrija respostas quando alguém mudar de planos.</p></section>{event&&<InviteManager eventId={event.id} invitations={invitations??[]} defaultOpen readOnly={readOnly}/>}<GuestEditor guests={data??[]} readOnly={readOnly}/></main>;
}
