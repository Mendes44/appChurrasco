...
export function GuestEditor({ guests }: { guests: Guest[] }) {
  ...
  return <section className="card guest-admin">
    ...
    {editing && <form className="admin-form edit-response" id="editar-convidado" ref={editFormRef} key={editing.id} onSubmit={save}>
      ...
      <label>Nome do acompanhante<input name="companionName" defaultValue={editing.companion_name??""}/></label>
      <label>Presença e total<select name="partySize" defaultValue={editing.party_size}><option value="0">Não vai</option><option value="1">Vai sozinho</option><option value="2">Vai com acompanhante</option></select></label>
      <label>Quantas pessoas bebem?<select name="drinkersCount" defaultValue={editing.drinkers_count}><option value="0">Nenhuma</option><option value="1">1 pessoa</option><option value="2">2 pessoas</option></select></label>
