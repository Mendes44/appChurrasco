...
export async function POST(request: Request) {
  ...
  const body = await request.json(); const token = String(body.token ?? ""); const name = String(body.name ?? "").trim(); const phone = String(body.phone ?? "").replace(/\D/g, "");
  if (!/^[0-9a-f-]{36}$/i.test(token) || name.length < 2 || name.length > 80 || phone.length < 10 || phone.length > 13) return NextResponse.json({ message: "Revise os dados." }, { status: 400 });
