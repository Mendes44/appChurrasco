...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  // Quantidades são limitadas ao máximo de duas pessoas definido pelo convite.
  ...
  if (name.length < 2 || name.length > 80 || (phone && (phone.length < 10 || phone.length > 13))) return NextResponse.json({ message:"Informe nome e telefone válidos." }, { status:400 });
  const companion = party === 2 ? String(body.companionName ?? "").trim() : null;
  const attended = body.attended === "yes" ? true : body.attended === "no" ? false : null;
