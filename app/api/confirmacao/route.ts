...
export async function POST(request: Request) {
  try {
    ...
    if (typeof body.token !== "string" || !/^[0-9a-f-]{36}$/i.test(body.token) || (body.attending&&(phone.length < 10 || phone.length > 13)) || companionName.length > 80 || typeof body.attending !== "boolean" || typeof body.primaryDrinks !== "boolean" || typeof body.companionDrinks !== "boolean" || typeof body.bringsOwnDrink !== "boolean") {
    ...
    const { data: event } = await supabase.from("events").select("status,rsvp_deadline").eq("id", invitation.event_id).maybeSingle();
    ...
    const { error } = await supabase.rpc("submit_personal_invitation", {
      ...
      p_phone: body.attending ? phone : null,
    });
