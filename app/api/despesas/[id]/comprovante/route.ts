...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const{data:event}=await context.database.from("events").select("id,status").eq("id",expense.event_id).eq("owner_id",context.user.id).maybeSingle();if(!event)return NextResponse.json({message:"Não autorizado."},{status:403});
